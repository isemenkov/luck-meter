...
final class LuckyFormTest extends TestCase
{
    ...
    public function testLuckyFormSuccess(): void
    {
        $this->app->instance(
        ...
        $routeUrl = route('lucky_form', ['access_token' => self::ACCESS_TOKEN]);
        $response = $this->get($routeUrl);

        $response->assertStatus(200);
        $response->assertSee('Luck Meter');
        $response->assertSee('Score');
        $response->assertSee('Show History');
    }

    public function testLuckyCalculateSuccess(): void
    {
        $this->app->instance(
            AccessTokenRepository::class,
            Mockery::mock(AccessTokenRepository::class, function (Mockery\MockInterface $mock) {
        ...
        $routeUrl = route('luck-calculator');
